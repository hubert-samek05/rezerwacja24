...
export default function EditEmployeePage() {
  ...
  const loadData = async () => {
    try {
      setLoadingData(true)
      const data = await employeesApi.getOne(employeeId)
      setFormData({
    ...
    <div className="p-6 lg:p-8">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="mb-6">
          ...
          <h1 className="text-2xl font-semibold text-[var(--text-primary)]">Edytuj pracownika</h1>
          <p className="text-sm text-[var(--text-muted)] mt-1">{formData.firstName} {formData.lastName}</p>
        </div>
