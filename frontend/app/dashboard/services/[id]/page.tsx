...
import { ArrowLeft, Save, Loader2, Check, Upload, X, Image as ImageIcon } from 'lucide-react'
  ...
  const loadData = async () => {
    try {
      ...
      setFormData({
        ...
        duration: serviceData.duration?.toString() || '60',
      ...
      if (serviceData.service_employees?.length) {
        setSelectedEmployees(serviceData.service_employees.map((se: any) => se.employeeId).filter(Boolean))
      }
    } catch (error) {
    ...
    <div className="p-6 lg:p-8">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="mb-6">
          ...
          <h1 className="text-2xl font-semibold text-[var(--text-primary)]">Edytuj usługę</h1>
          <p className="text-sm text-[var(--text-muted)] mt-1">{formData.name}</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl">
          ...
          <div className="p-5 border-b border-[var(--border-color)]">
            ...
            {employees.length === 0 ? (
              <p className="text-sm text-[var(--text-muted)]">Brak pracowników.</p>
            ) : (
