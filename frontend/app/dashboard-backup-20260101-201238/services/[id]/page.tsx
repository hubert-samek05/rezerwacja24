...
export default function EditServicePage() {
  ...
  const loadData = async () => {
    try {
      ...
      setFormData({
        ...
        bufferBefore: serviceData.bufferBefore?.toString() || '0',
        bufferAfter: serviceData.bufferAfter?.toString() || '0',
        maxCapacity: serviceData.maxCapacity?.toString() || '1',
        requiresDeposit: serviceData.requiresDeposit || false,
        depositAmount: serviceData.depositAmount?.toString() || '0',
      ...
      if (serviceData.service_employees && Array.isArray(serviceData.service_employees)) {
        const employeeIds = serviceData.service_employees.map((se: any) => se.employeeId).filter(Boolean)
        setSelectedEmployees(employeeIds)
      } else if (serviceData.employees && Array.isArray(serviceData.employees)) {
        // Fallback dla starego formatu
        const employeeIds = serviceData.employees.map((se: any) => se.employeeId || se.employee?.id).filter(Boolean)
        setSelectedEmployees(employeeIds)
      }
    } catch (error) {
      console.error('Błąd ładowania danych:', error)
      toast.error('Nie udało się załadować danych usługi')
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(error.response?.data?.message || 'Nie udało się zaktualizować usługi')
    } finally {
      setLoading(false)
    }
  }
    ...
    return (
      <div className="min-h-screen bg-carbon-black flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-neutral-gray">Ładowanie danych usługi...</p>
        </div>
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-carbon-black p-8">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <h1 className="text-3xl font-bold text-white">Edytuj usługę</h1>
          <p className="text-neutral-gray/70 mt-2">Zaktualizuj informacje o usłudze</p>
        </div>
