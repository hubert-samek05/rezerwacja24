...
export default function SubscriptionTab() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-xl sm:text-2xl font-bold text-white mb-2">Subskrypcja</h2>
      ...
      {!subscription ? (
        ...
        <motion.div
          ...
          animate={{ opacity: 1, y: 0 }}
          className="bg-gradient-to-br from-[#0B2E23] to-[#0F6048] rounded-xl sm:rounded-2xl p-4 sm:p-8 border border-[#41FFBC]/20"
        >
          <div className="text-center mb-6 sm:mb-8">
            <Sparkles className="w-10 h-10 sm:w-12 sm:h-12 text-[#41FFBC] mx-auto mb-3 sm:mb-4" />
            <h3 className="text-2xl sm:text-3xl font-bold text-white mb-3 sm:mb-4">Plan Pro</h3>
            <div className="flex items-baseline justify-center gap-2 mb-3 sm:mb-4">
              <span className="text-3xl sm:text-5xl font-bold text-[#41FFBC]">79,99 zł</span>
              <span className="text-lg sm:text-xl text-gray-300">/miesiąc</span>
            </div>
            <p className="text-sm sm:text-lg text-gray-300 mb-2">Pełny dostęp do wszystkich funkcji</p>
            <p className="text-xs sm:text-sm text-[#41FFBC] font-semibold">7 dni okresu próbnego za darmo</p>
            <p className="text-xs sm:text-sm text-gray-400 mt-2">Wymagana karta płatnicza</p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3 mb-6 sm:mb-8">
            {[
              ...
              <div key={index} className="flex items-center gap-2">
                <CheckCircle2 className="w-4 h-4 text-[#41FFBC] flex-shrink-0" />
          ...
          <button
            ...
            className="w-full bg-[#41FFBC] hover:bg-[#41FFBC]/90 text-[#0A0A0A] font-bold py-3 px-4 sm:px-6 rounded-lg sm:rounded-xl transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 text-sm sm:text-base"
          >
            {actionLoading ? (
              <>
        ...
        <div className="space-y-6">
          ...
          >
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-6">
              <div>
                <h3 className="text-lg sm:text-xl font-bold text-white mb-2">{subscription.subscription_plans?.name || 'Plan Pro'}</h3>
              ...
              <div className="text-left sm:text-right">
                <div className="text-xl sm:text-2xl font-bold text-[#41FFBC]">
                  {subscription.subscription_plans?.priceMonthly || '79.99'} {subscription.subscription_plans?.currency || 'PLN'}
                </div>
                <div className="text-xs sm:text-sm text-gray-400">miesięcznie</div>
              </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <button
                ...
                disabled={actionLoading}
                className="flex-1 bg-primary-green hover:bg-primary-green/80 text-white font-semibold py-3 px-4 rounded-lg transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 text-sm sm:text-base"
              >
                {actionLoading ? (
                  <Loader2 className="w-5 h-5 animate-spin" />
                ) : (
              ...
              {subscription.cancelAtPeriodEnd ? (
                <button
                  ...
                  disabled={actionLoading}
                  className="flex-1 bg-green-600 hover:bg-green-700 text-white font-semibold py-3 px-4 rounded-lg transition-all disabled:opacity-50 disabled:cursor-not-allowed text-sm sm:text-base"
                >
                  Wznów subskrypcję
                </button>
              ) : (
            ...
            >
              ...
              <div className="space-y-3">
                {invoices.map((invoice) => (
                  <div
                    key={invoice.id}
                    className="bg-[#0A0A0A]/50 rounded-lg p-3 sm:p-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3"
                  >
                    <div>
                      <div className="text-white font-semibold">
